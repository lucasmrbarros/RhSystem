...
dropdown_vaga.append('<option selected="true">Vaga</option>');
...
$.getJSON(url_vaga, function (data) {
    $.each(data, function (_key, entry) {
        dropdown_vaga.append($('<option></option>').attr('value', entry.abbreviation).text(entry.nome));
    })
});
...
$.getJSON(url_tecnologia_drop, function (data) {
    $.each(data, function (_key, entry) {
        dropdown_tecnologia.append($('<option></option>').attr('value', entry.abbreviation).text(entry.nome));
    })
});
...
$.getJSON(url_tecnologia, function (data) {
    $.each(data, function (_key, entry) {
        checkbox_tecnologias.append($('<input type="checkbox">  '));
